...
function addRemainingSwaggerProperties(swagger, projectJson) {
	...
	if (projectJson.securityDefinitions) {
	  swagger.securityDefinitions = projectJson.securityDefinitions;

    // This property should be added only if there are security definitions to avoid issues in spec validation
    if (projectJson.security) {
      swagger.security = projectJson.security;
    }
  }
}
 ...
 * @param swagger			Swagger specification object where API path will be added
 * @param apidocJson
 * @returns {{}}
 */
function extractPaths(swagger, apidocJson){
	...
	for (var i = 0; i < apiPaths.length; i++) {
		...
		for(var j = 0; j < verbs.length; j++) {
			...
			if (!url) {
				// This path doesn't have to be created since it's null/undefined or empty
				// Check if there is any Model defined and add to swagger definitions
				createVerbDefinitions(verb, swagger.definitions);
				continue;
			}
...
function createPostPushPutOutput(verb, definitions, pathKeys) {
  ...
  var consumes = verb.consumes && Array.isArray(verb.consumes) && verb.consumes.length > 0 ? verb.consumes : ["application/json"],
      produces = verb.produces && Array.isArray(verb.produces) && verb.produces.length > 0 ? verb.produces : ["application/json"];

  if ((consumes.indexOf("application/json") > -1) || (consumes.indexOf("application/xml") > -1)) {
    // TODO: This condition should be changed or removed since it's assuming that parameters will be sent in body. Also it's considering only the first defined "Parameter".
    var required = verb.parameter && verb.parameter.fields && verb.parameter.fields.Parameter && verb.parameter.fields.Parameter.length > 0 && !verb.parameter.fields.Parameter[0].optional;
    params.push({
      "in": "body",
      "name": "body",
...
function createSwaggerResponse(apiDocResponse) {
  var swaggerResponse = {},
	...
	swaggerResponse.description = apiDocResponse.description ? apiDocResponse.description : "";
...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
    ...
    // TODO: replace all 'parameter.type.toLowerCase' with a local variable
		...
		if (!nestedName.objectName) {
		  // If the objectName from the Parameter is empty/null, then a new definition shouldn't be created
      // If this conditional is removed, then fictitious definitions based on the endpoint name are created which
      // is a big issue if swagger-codegen module is used
			continue;
		}
      ...
      if(typeIsArray(parameter.type)) {
        ...
        if (parameter.type.toLowerCase().indexOf("object") > -1) {
          ...
          var swaggerType = findSwaggerDataType(parameter.type.slice(0, parameter.type.length - 2));
          prop.items = {
...
function createGetDeleteOutput(verb, definitions) {
  ...
  var consumes = verb.consumes;
  var produces = verb.produces;

	pathItemObject[verb.type] = {
		tags: [verb.group],
    summary: removeTags(verb.title),
    description: verb.description,
		consumes: consumes && Array.isArray(consumes) && consumes.length > 0 ? consumes : ["application/json"],
    produces: produces && Array.isArray(produces) && produces.length > 0 ? produces : ["application/json"],
		parameters: params
	};
